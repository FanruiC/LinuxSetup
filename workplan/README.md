...
> 
> 但是除了质心之外的质点该如何运动呢？如果我们研究刚体，则大大简化，就好像宇航员，物品全部冻结在飞船的某个位置，形成刚体一样。当合外力为零时候，the total momentum of a collection of particles is 0， 但这并不意味着所有粒子都不运动，恰恰相反，所有粒子都在绕一个穿过质心的轴转动，每个粒子都有自己的动量，且动量还在不停转变方向，只不过在计算合动量时候，total momentum全部抵消掉了而已。既然合动量为零（或不变）， 那我们如何描述这种旋转的程度呢？我们首先从一个粒子开始：
...
> 发现这个L恰恰是 $xmv_y - ymv_x$, 即 $L = xP_y - y_Px$ 称作角动量（在某一瞬时时刻，绕曲率半径为R的旋转轴！）。
